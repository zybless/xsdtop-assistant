...
当前发行版覆盖数学与物理内容工作流，支持查询整理、分组与容器管理、题目和题单编辑、批量公开隐藏、图片上传、录入前校验和确认入库。相同结构可以继续承载新的业务能力，也可以作为设计本地 AI 插件时的实现参考。

PDF、Word 和扫描图片录题通过本地 `mineru-open-mcp` 进程直接读取文件路径，再调用 MinerU 精准解析接口，不需要网页中转上传。使用这项能力前，系统需要能够执行 `uvx`，并已配置 MinerU 个人密钥。
...
- 本地文档由本机 MinerU MCP 进程读取并发送至 MinerU 精准解析接口；插件不会自动打开浏览器或模拟鼠标上传。
